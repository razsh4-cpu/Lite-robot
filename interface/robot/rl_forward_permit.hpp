...
// One-shot bounded forward-test grant. This is operator authorization, never
// evidence of MotionSDK ownership, and it cannot be renewed or reused.
class RLForwardPermit {
    ...
    RLForwardPermit(std::shared_ptr<const std::atomic<bool>> abort,const std::atomic<bool>* signal)
      :abort_(std::move(abort)),signal_(signal),expires_(Clock::now()+std::chrono::seconds(5)),
...
public:
    const char* FailureReason() const {
        ...
        if(abort_->load() || (signal_&&signal_->load()))return "RL forward permit cancelled";
        if(now>=expires_)return "RL forward 5s deadline";
        if(now-Clock::time_point(Clock::duration(last_send_.load()))>=std::chrono::milliseconds(300))
            return "RL forward policy-send timeout 300ms";
        return nullptr;
    }
